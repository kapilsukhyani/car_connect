...
internal class FuelAndTemperatureGaugeView(context: Context,
                                           ...
                                           onlineColor: Int,
                                           offlineColor: Int,
                                           gaugeBackgroundDrawable: Drawable) : RightGaugeView(context,
        attrs,
        defStyleAttr,
        defStyleRes,
        onlineColor,
    ...
    private val fuelIcon = VectorDrawableCompat.create(context.resources,
    ...
    internal fun onBoundChanged(bounds: RectF,
                                middleGaugeRadius: Float,
                                sideAndMiddleGaugeIntersectionLength: Float) {
...
//                onFuelIconCLickListener?.invoke(fuelPercentage)
                true
            }
            ambientTouchBounds.contains(event.x, event.y) -> {
//                onAmbientTempClickListener?.invoke(currentAmbientTemp)
                true
            }
            airIntakeTouchBounds.contains(event.x, event.y) -> {
//                onAirIntakeTempClickListener?.invoke(currentAirIntakeTemp)
